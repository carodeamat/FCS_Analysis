...
There are two main methods for clustering flow data, FlowSOM and Phenograph. \
...
The following code will perfom phenograph clustering to the data considering the previously selected markers. \
The results depend on the indicated k parameter, which can be adjusted.
...
# Adjust k as needed
pheno <- Rphenograph(scale(data$expr.mat), k = k_param) 

#Insert the clustering information into the data
data$metadata$Clusters  <- as.factor(membership(pheno[[2]]))
...
AnalysisID <- Sys.Date()
filename <- paste("Phenograph_clustering_", seed, "_", AnalysisID, ".rds", sep="")
